    ...
    let filterSymbolUses (getSourceLineStr: int -> string) (symbolUses: FSharpSymbolUse[]) : FSharpSymbolUse[] =
        ...
        |> Array.filter (fun su ->
             ...
             | :? FSharpMemberOrFunctionOrValue as fv when fv.IsExtensionMember -> 
                ...
                true
             | _ ->
                let line = su.RangeAlternate.StartLine
                let lineText = getSourceLineStr line
                let partialName = QuickParse.GetPartialLongNameEx (line, su.RangeAlternate.EndColumn - 1, lineText)

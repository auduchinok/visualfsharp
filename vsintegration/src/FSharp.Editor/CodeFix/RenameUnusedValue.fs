    ...
    override __.RegisterCodeFixesAsync context : Task =
        asyncMaybe {
            ...
            if not (PrettyNaming.IsOperatorOrBacktickedName ident) && not (ident.StartsWith "``") then
                let! _, projectOptions = projectInfoManager.TryGetOptionsForEditingDocumentOrProject document
                ...
                let! symbolUse = checkResults.GetSymbolUse(m.StartLine, m.EndColumn, lineText, userOpName=userOpName)

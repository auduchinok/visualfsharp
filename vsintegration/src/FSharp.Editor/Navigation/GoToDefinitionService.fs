...
type internal GoToDefinition(checker: FSharpChecker, projectInfoManager: FSharpProjectOptionsManager) =
    ...
    let findSymbolHelper (originDocument: Document, originRange: range, sourceText: SourceText, preferSignature: bool) : Async<FSharpNavigableItem option> =
        asyncMaybe {
            ...
            let! fsSymbolUse = checkFileResults.GetSymbolUse(fcsTextLineNumber, idRange.EndColumn, lineText, userOpName=userOpName)
...
type internal FSharpGoToDefinitionService 
    ...
    member __.FindDefinitionsTask(originDocument: Document, position: int, cancellationToken: CancellationToken) =
        asyncMaybe {
            ...
            let! declarations = checkFileResults.GetDeclarationLocation(fcsTextLineNumber, lexerSymbol.Ident.idRange.EndColumn, textLine.ToString(), preferSignature, userOpName=userOpName) |> liftAsync
            let! targetSymbolUse = checkFileResults.GetSymbolUse(fcsTextLineNumber, idRange.EndColumn, lineText, userOpName=userOpName)
            ...
            | FSharpFindDeclResult.DeclFound targetRange -> 
                ...
                if lexerSymbol.Range = targetRange then
                    ...
                    if isSignatureFile originDocument.FilePath then
                        ...
                        let! declarations = checkFileResults.GetDeclarationLocation (fcsTextLineNumber, lexerSymbol.Ident.idRange.EndColumn, textLine.ToString(), true, userOpName=userOpName) |> liftAsync
                        match declarations with
                        | FSharpFindDeclResult.DeclFound targetRange -> 
